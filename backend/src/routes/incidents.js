...
router.post('/', auth, async (req, res) => {
  const { fecha, turno, tipo, descripcion, prioridad, responsable_id, fecha_compromiso, evidencia_url } = req.body;
  ...
  db.data.incidents.push({ id, branch_id: 1, fecha, turno: turno || null, reportado_por: req.user.id, tipo, descripcion, prioridad: prioridad || 'media', responsable_id: responsable_id || null, fecha_compromiso: fecha_compromiso || null, estatus: 'pendiente', evidencia_url: evidencia_url || null, created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
  await db.write();
